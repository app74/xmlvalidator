...
export function getXmlTagMatches(xml: string, tagName: string): Array<{ value: string; line: number; column: number; offset: number }> {
  const regex = new RegExp(`<(?:[A-Za-z_][A-Za-z0-9_.-]*:)?${tagName}\\b[^>]*>([\\s\\S]*?)<\\/(?:[A-Za-z_][A-Za-z0-9_.-]*:)?${tagName}>`, 'gi');
  const matches: Array<{ value: string; line: number; column: number; offset: number }> = [];
...
export function getXmlTagOccurrences(xml: string, tagName: string): { value: string; line: number; column: number }[] {
...
export function findSuspiciousCharacters(xml: string): ValidationIssue[] {
